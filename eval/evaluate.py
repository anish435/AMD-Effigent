...
def run_evaluation(tasks_path: str) -> dict:
    ...
    for item in task_data:
        ...
        tasks.append(Task(id=str(tid), prompt=prompt, expected_answer=expected))
    ...
    # Save self-calibration analysis
    misrouted_tasks = [
        {
            "task_id": t["task_id"],
            ...
            "reason": "Routed LOCAL but escalated to REMOTE"
        }
        for t in results
        if t["route"] == "local" and t["fallback"]
    ]
    try:
        analysis_path = Path("router_analysis.json")
        analysis_path.write_text(
            json.dumps({
                "misrouted_count": len(misrouted_tasks),
                "total_tasks": len(results),
                "misrouted_percentage": f"{len(misrouted_tasks) / len(results) * 100:.1f}%" if results else "0.0%",
                "misrouted_tasks": misrouted_tasks
            }, indent=2), encoding="utf-8"
        )
    except Exception as e:
        print(f"Failed to save router_analysis.json: {e}")
